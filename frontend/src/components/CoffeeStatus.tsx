...
function getStatus(brews: Brew[]): StatusInfo {
  if (brews.length === 0) {
    return { label: 'NO BREWS YET', level: 'idle', dotClass: 'dot--idle', minutesAgo: null };
  }
  const latest = brews[0];
  const minutesAgo = (Date.now() - new Date(latest.brewedAt).getTime()) / 60_000;

  if (minutesAgo < 5) {
    return { label: 'BREWING', level: 'brewing', dotClass: 'dot--brewing', minutesAgo };
  }
  if (minutesAgo < 30) {
    return { label: 'FRESH', level: 'fresh', dotClass: 'dot--fresh', minutesAgo };
  }
  if (minutesAgo < 90) {
    return { label: 'GETTING OLD', level: 'getting-old', dotClass: 'dot--old', minutesAgo };
  }
  return { label: 'MAKE MORE', level: 'make-more', dotClass: 'dot--stale', minutesAgo };
}
